...
func (h *handlerCart) AddToCart(w http.ResponseWriter, r *http.Request) {
	...
	cartExist, err := h.CartRepository.GetCartByUser(userId, productId)
...
func (h *handlerCart) GetCartByUserID(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")

	userInfo := r.Context().Value("userInfo").(jwt.MapClaims)
	userId := int(userInfo["id"].(float64))

	cart, err := h.CartRepository.GetCartByUserID(userId)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		response := dto.ErrorResult{Code: http.StatusBadRequest, Message: err.Error()}
		json.NewEncoder(w).Encode(response)
...
func (h *handlerCart) DeleteCartByQty(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	cartItem, _ := h.CartRepository.GetCartByUser(userId, productId)
