...
func (h *handlerTransaction) GetTransactionByPartner(w http.ResponseWriter, r *http.Request) {
	...
	response := dto.SuccessResult{Code: "Success", Data: h.convertTransactionResponse(myTransaction)}
	json.NewEncoder(w).Encode(response)
...
func (h *handlerTransaction) CreateTransaction(w http.ResponseWriter, r *http.Request) {
	...
	if validateErr != nil {
		w.WriteHeader(http.StatusInternalServerError)
		response := dto.ErrorResult{Code: http.StatusBadRequest, Message: validateErr.Error()}
		json.NewEncoder(w).Encode(response)
		return
	}
	...
	if createTransactionErr != nil {
		w.WriteHeader(http.StatusInternalServerError)
		response := dto.ErrorResult{Code: http.StatusBadRequest, Message: createTransactionErr.Error()}
		json.NewEncoder(w).Encode(response)
		return
	}
	...
	userCart, err := h.TransactionRepository.FindCartByUserID(userId)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		response := dto.ErrorResult{Code: http.StatusBadRequest, Message: err.Error()}
		json.NewEncoder(w).Encode(response)
