...
type CartRepository interface {
	...
	GetCartByUser(userID int, productID int) (models.Cart, error)
...
func (r *repository) GetCartByUser(userID int, productID int) (models.Cart, error) {
	var cart models.Cart
	err := r.db.Preload("Users").Preload("Products").Where("users_id = ? and product_id=?", userID, productID).First(&cart).Error
	return cart, err
}

func (r *repository) GetCartByUserID(userID int) ([]models.Cart, error) {
	var cart []models.Cart
